    ...
    <nav style={{
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 24px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none' }}>
          <div style={{ width: 34, height: 34, borderRadius: 10, background: 'linear-gradient(135deg, #F59E0B, #FCD34D)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 800, color: '#000', fontFamily: 'var(--font-display)' }}>A</div>
          <div>
          ...
          {navLinks.map(link => {
            ...
            return (
              <Link key={link.href} href={link.href} style={{
                ...
                color: active ? '#F59E0B' : 'rgba(255,255,255,0.55)', textDecoration: 'none',
              ...
              onMouseLeave={e => { if (!active) { (e.target as HTMLElement).style.color = 'rgba(255,255,255,0.55)'; (e.target as HTMLElement).style.background = 'transparent'; }}}
              >
                {link.label}
              </Link>
            );
      ...
      {open && (
        <div style={{ background: '#0C0C0E', borderTop: '1px solid rgba(255,255,255,0.06)', padding: '12px 24px 20px' }} className="md:hidden">
          {navLinks.map(link => (
            <Link key={link.href} href={link.href} onClick={() => setOpen(false)} style={{ display: 'block', padding: '12px 0', fontSize: 15, fontWeight: 500, color: 'rgba(255,255,255,0.6)', textDecoration: 'none', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>{link.label}</Link>
          ))}
          <Link href="/estimate" onClick={() => setOpen(false)} className="btn-primary" style={{ marginTop: 12, justifyContent: 'center', width: '100%', display: 'flex' }}>
            Free Estimate
          </Link>
        </div>
      )}
